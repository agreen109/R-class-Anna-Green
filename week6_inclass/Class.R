...
#Because if using functions later, they need to be made 
...
collect_images <- function(im.path) {
  ...
  file_names <- fs::dir_ls(im.path) #list file names 
...
calc_area_by_thresholds <- function(in.raster,
  ...
  # What's a CRS again? #coordinate refrence system 
  ...
  # raster properties
  raster.res <- terra::res(in.raster) #res->resolution
  pixel.area.m2 <- raster.res[1] * raster.res[2] #gives x,y
  ...
  masked_raster <- mask(ras.subset, ras.subset >= 2 &
                          ras.subset <= 249) #value must be greater than or equal to 2 and less than or equal to 249
  ...
  cell.count <- terra::global(ras.subset, fun = "sum", na.rm = T) #means to ignore functions that do not have a value
