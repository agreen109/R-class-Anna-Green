...
rclr<- matrix(c(0,0,NA,1,249,1,250,252,NA),ncol=3,byrow=TRUE)
rclr
...
Using `ggplot2`, create a line or point plot showing mean CI values over time. Label your axes clearly. Which date in your dataset shows the highest mean bloom intensity? September 20th, 2017 had the highest intesity
...
shapefiles_needed<-list.files(path="erie_raster_ex/bounds", pattern="\\.shp$",full.names=TRUE)
temp_list<-list()
for (i in 1:length(shapefiles_needed)) {
  current_shp<-sf::read_sf(shapefiles_needed[i])+
    current_shp$AOI<-basename(shapefiles_needed[i])+
      temp_list[[i]]<-current_shp
}
