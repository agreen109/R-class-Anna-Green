...
Use `fs::dir_ls` with the parameter `regexp = "\\.tif$"` to get the file paths for all raster images in the `images/` directory. Store the result in an object called `raster_files`. Print the result — how many files are there? There are 7 files
...
Load the first raster in your list using `terra::rast()`. Examine the following properties and record what you find: min=1 and max=252 for my single raster 
...
terra::global(single_raster,"min", na.rm=TRUE)
terra::global(single_raster, "max", na.rm=TRUE)
```
...
clean_cicyano <- function(r){
  reclass_matrix<- matrix(c(0,0,NA,
                          ...
                          ncol=3,
                          byrow=TRUE)
raster_cleaned<-classify(r, reclass_matrix,right=NA)
return(raster_cleaned)
}

reclass<- matrix(c(0,0,NA,1,249,1,250,252,NA),ncol=3,byrow=TRUE)
reclass

raster_cleaned<-single_raster #test to compare the raw raster vs cleaned raster 
raw_raster<-terra::global(single_raster, "range", na.rm=TRUE)
cleaned_raster<-terra::global(raster_cleaned, "range",na.rm=TRUE)
print(raw_raster)
print(cleaned_raster)
```
...
dn_to_ci_test<-dn_to_ci(raster_cleaned) #test to versify function 
terra::minmax(dn_to_ci_test)
```
...
The CRS of these rasters tells you about how distances and areas are measured by giving you the units of measurements within your data. You would need to reproject before calculating the bloom area in square kilometers, which makes sure that your units are in meters before calculating the bloom area in square kilometers. 
...
It is important to remove sentinel values before applying the DN-to_CI transform rather than after because the sentinel values are not measurements but rather are marks on land that stand for a specific place on land. If you transformed a land pixel with a value of 252 without cleaning first, the DN-to-CI transform would treat that number as a real measurement, which would not make your data correct. 

**Q1.3** Why is the `right = NA` argument useful in `terra::classify()` for this task?
The right=NA argument is useful in terra::classify() for this task because it allows for you to select the variables you want to use. In 1.3, we used this function so that the boundaries 0 and 250 were included in the intervals, so this function is useful when wanting to use spepcific points in classifying a data set. 
---
...
area_interest<-sf::read_sf("erie_raster_ex/bounds/bb_erie_sandusky.shp")
print(sf::st_crs(area_interest))
print(terra::crs(single_raster))
area_reproject<-sf::st_transform(area_interest,terra::crs(single_raster))
glimpse(area_reproject)
glimpse(area_interest)
```
...
august_ras<-terra::rast("erie_raster_ex/images/ts_2017.0816_0822.L4.LE3.CIcyano.MAXIMUM_7day.tif")
august_cleaned<-clean_cicyano(august_ras)
august_ci<-dn_to_ci(august_cleaned)
august_crop<-terra::crop(august_ci,area_reproject)
sandusky_0816<-terra::mask(august_crop,area_reproject)
plot(sandusky_0816) #to visualize and check code
```
...
mean_LE<-terra::global(august_ci, "mean",na.rm=TRUE)
...
tm_shape(sandusky_0816)+
  ...
  tm_layout(main.title="Cyanobacteria blooms in Sandusky Bay August 2017",
            main.title.size=1,
            lengend.outside=TRUE)+
...
terra::crop reduces the raster into smaller bounding boxes, whereas a terra::mask changes the value of the cells to either true or NA, making points that fall out of the shapes NA and everything inside the cells true. We used both here instead of just one because crop gives us more precise data boxes, then after we have a more precise bounding box we can use terra:mask to highlight the area of interest and assign NA to areas outside of our area of interest.
...
The mean CI value for Sandusky bay is likely different from the full lake mean because of differences in volumes and areas being studied. Sandusky bay is a smaller portion of the lake, so although it may have the same amount of cyanobacteria within it as the full lake, because the area of the bay is much smaller than the full lake, the cyanobacteria bloom seems to be larger in the bay. THis does not mean that Sandusky Bay has more or less cyanobacteria than average, but rather it could hold the same amount of cyanobacteria as the full lake but just in a smaller area. 
...
four_rasters<-raster_files[c(1,2,3,4)]
stack_rasters<-four_rasters %>%
  purrr::map(~{
    r<-terra::rast(.x)
    r<-clean_cicyano(r)
    r<-dn_to_ci(r)
...
mean_CI<-terra::global(stack_rasters, fun="mean", na.rm=TRUE)
dates<-four_rasters %>%
  basename() %>% stringr::str_extract("\\d{4}\\.\\d{4}")
print(dates)
combined_data<-data.frame(
  date=dates,
  mean_CI=mean_LE$mean
)
print(dates[[4]])
print(combined_data)
...
ggplot(mean_CI, aes(x=dates, y=mean))+
...
raster_early<-rast(four_rasters[[1]])
raster_late<-rast(four_rasters[[2]])
bloom_early<-terra::classify(raster_early, cbind(NA,0),others=1)
bloom_late<-terra::classify(raster_late, cbind(NA,0),others=1)
persistent_bloom<-(raster_early==1)&(raster_late==1)
new_bloom<-(bloom_late==1)&(bloom_early==0)
plot(persistent_bloom, main="persistant bloom",col=c("blue","lightblue"))
plot(new_bloom, main="new bloom", col=c("white", "blue"))
...
Using a mean CI value as a measuer of bloom severity across the whole lake can be a major limitation, as the mean CI value does not take into account outliers in the dataset values, as well as distribution of blooms within the lake. If the mean of the lake is a certain value, that does not mean the bloom is that value across the whole lake. A more robust alternative metric would be to use a meaure that could give you the surface area of the blooms to help gain an understanding of the coverage of the blooms on a lake, or a way to tell where the outliers on the lake are in terms of bloom serverity, so find areas that have the most servere blooms. 
...
All rasters must meet the requirements of having an identical bounding box, the same cell size, the same projection, and the same number of rows and columns in order to be stacked this way. 
---
...
area_interest <- terra::extract(august_ci, aoi_vec, 
...
summary_table<-all_aois %>%
  group_by(ID)%>%
  summarise(
    meanCI=mean(CI, na.rm=TRUE),
    maxCI=max(CI, na.rm=TRUE)
  )
glimpse(all_aois)
highest_mean<-summary_table%>%filter(meanCI==max(mean_CI))
highest_max<-summary_table%>%filter(maxCI==mean(maxCI))




summary_table <- bind_cols(all_aois, area_interest)
print(summary_table)
highest_mean <- summary_table %>% 
  st_drop_geometry() %>%
  slice_max(mean, n = 1)
```
...
tm_shape(area_interest) + 
  tm_raster(title = "CI Value")+ 
  tm_shape(shapefiles)+ 
  tm_borders(col = "lightblue")+ 
  tm_text("boundaries")+
  tm_layout(main.title = "CI raster 0920 with AOI boundaries overlaid", legend.outside = TRUE)
```
...
Getting NA values in the terra::extract results for some AOIs can be caused by missing data within the raster, which could result in NA since the data is not present, or if there are no overlaps in the AOI shape, this can also lead to NA results.  
...
I expect areas closer to the edge of the lake to be more prone to severe bloom because of exposure to runoff, and shallower waters which can in turn make for higher water temperatures within those areas. 
---
...
The analysis workflow in this lab was different then working with vector data because vector data are specific points in a data set within a cell or gird whereas rasters are composed of points, lines, and polygons. Because the vectors are not made up of one specific points but rather is a path, it is very different working with vectors then working with rasters.  
...
The timing of peak blooms from my four data points kinda goes opposite of what I would expect. In my data set, the blooms peaked from August to September. From what I know on algea blooms, I would expect the peak of the bloom to be in August when the temperatures are warmer than Spetember, but my dataset showed that the peak bloom was in September. 
...
Although I could not get the terra::extract to work in task 4 to perform a zonal summary, this can relate conceptually to the zonal operations category of map algebra discussed in class because the zonal operation of map algebra summarized the raster cell values based off the zone they are in. The terra::extract in task four also acts as a way to algebraically summarize raster cells based on the zones they are in within a shape, so this function is similar to the zonal operations we discussed in class. 
